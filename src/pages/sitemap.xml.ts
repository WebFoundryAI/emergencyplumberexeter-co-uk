...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Exeter gets higher priority)
    const isExeter = location.slug === 'exeter';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isExeter ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isExeter ? 1 : 2 + i),
    });
