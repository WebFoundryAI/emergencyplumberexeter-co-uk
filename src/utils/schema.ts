...
export function getBaseBusinessSchema() {
  return {
    ...
    "description": "Professional drain unblocking, CCTV surveys, and emergency drainage services across Exeter and surrounding areas. Fast response, no call-out fee, 24/7 availability.",
    "url": siteUrl,
    ...
    "areaServed": ALL_SERVICE_AREAS.map(area => ({
      ...
      "containedInPlace": {
        ...
        "name": "Exeter and surrounding areas"
      }
    })),
    "hasOfferCatalog": {
      "@type": "OfferCatalog",
...
export function getLocationPageSchema(location: Location) {
  ...
  const locationSchema = {
    ...
    "areaServed": {
      ...
      "containedInPlace": {
        "@type": "AdministrativeArea",
        "name": location.countyOrRegion || "Exeter and surrounding areas"
      }
    },
    "geo": {
      "@type": "GeoCoordinates",
...
export function getLocationServicePageSchema(location: Location, service: Service) {
  ...
  const serviceSchema = {
    ...
    "description": `Professional ${service.name.toLowerCase()} services in ${location.name}, ${location.countyOrRegion || 'Exeter and surrounding areas'}. Fast response, no call-out fee, 24/7 availability.`,
...
export function getWebSiteSchema() {
  return {
    ...
    "url": siteUrl,
    "name": BRAND.brandName,
    "description": "Professional drain unblocking, CCTV surveys, and emergency drainage services across Exeter and surrounding areas.",
    "publisher": {
      "@id": `${siteUrl}/#organization`
    },
    "inLanguage": "en-GB",
